...
{
    if(circle.getPosition().y >= WINDOW_Y - 20)  // Rebond sur bas de la fenetre but joueur 2
	{
        if(m_speed <= 0)
		{
            m_y = m_speed;
    ...
    if(circle.getPosition().y <= 0)	//Rebond sur le haut de la fênetre but joueur 1
	{
        if (m_speed <= 0)
            m_y = -1*m_speed;
        else
    ...
    if(circle.getPosition().x >= rectangle2.getPosition().x - rectangle2.getSize().x && circle.getPosition().y >= rectangle2.getPosition().y && //Rebond rectangle 2
